...
//realiza una búsqueda a través de los parámetros señalados
    ...
    @Query(value = "SELECT c FROM Consulta c WHERE c.profesional = :profesional", nativeQuery=true)
    public List<Consulta> buscarPorProfesional(@Param("profesional") Profesional profesional);
    
    @Query(value = "SELECT c FROM Consulta c WHERE c.paciente = :paciente", nativeQuery=true)
    ...
    @Query(value = "SELECT c FROM Consulta c WHERE c.estado = :estado", nativeQuery=true)
